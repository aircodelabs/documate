...
async function startChat(question: string) {
  ...
  if (!props.endpoint) {
    alert('Props endpoint is not provide')
...
<template>
<div>
  <span class="ask-ai">{{ props.askAILabel }}</span>
  <div class="Documate" v-show="open">
    <div class="dialog-mask" @click="() => onOuterClick()"></div>
      <div class="dialog-panel ease-out-enter" enter="ease-out duration-300" enter-from="opacity-0 scale-95" enter-to="opacity-100 scale-100" leave="ease-in duration-200" leave-from="opacity-100 scale-100" leave-to="opacity-0 scale-95">
        <div class="input-section">
          ...
          <input type="text" class="chat-input" :placeholder="props.placeholder" @keyup.enter="keyEnter" @input="event => query = (event.target as HTMLInputElement).value" autocomplete="off" :value="query" />
        </div>
        <div class="quick-options-section" v-if="!questions.length" static>
          <h2 class="quick-options-title">Frequently Asked</h2>
          <ul class="quick-option">
            <li class="quick-option-item" v-for="action in props.quickActions" :key="action" :value="action" as="template" @click="() => onSelect(action)">
              ...
              <span class="option-name">{{ action }}</span>
            </li>
          </ul>
        </div>

        <ul static class="question-anwser-section" ref="chatContainer">
        ...
        <div class="footer">
          ...
          <a href="https://documate-git-feature-docs-aircode.vercel.app/" class="powered-by">
            ...
            <path d="M122.054 37.8833C120.3 37.8833 118.727 37.451 117.335 36.5865C115.966 35.722 114.873 34.5453 114.057 33.0563C113.264 31.5674 112.868 29.8984 112.868 28.0493C112.868 26.2001 113.264 24.5311 114.057 23.0422C114.873 21.5533 115.966 20.3766 117.335 19.512C118.727 18.6475 120.3 18.2152 122.054 18.2152C123.735 18.2152 125.259 18.5635 126.628 19.2599C128.021 19.9323 129.078 20.8569 129.798 22.0336L127.745 24.5551C127.361 24.0028 126.868 23.4985 126.268 23.0422C125.668 22.5859 125.031 22.2257 124.359 21.9615C123.687 21.6974 123.038 21.5653 122.414 21.5653C121.261 21.5653 120.228 21.8535 119.316 22.4298C118.427 22.9822 117.719 23.7506 117.191 24.7352C116.662 25.7198 116.398 26.8245 116.398 28.0493C116.398 29.274 116.674 30.3787 117.227 31.3633C117.779 32.3239 118.511 33.0924 119.424 33.6687C120.336 34.2451 121.345 34.5333 122.45 34.5333C123.098 34.5333 123.723 34.4252 124.323 34.2091C124.947 33.9929 125.548 33.6687 126.124 33.2365C126.7 32.8042 127.241 32.2759 127.745 31.6515L129.798 34.173C129.03 35.2537 127.925 36.1422 126.484 36.8387C125.067 37.5351 123.59 37.8833 122.054 37.8833Z" fill="var(--dm-text-color)"/>
...
.dialog-panel {
  ...
  box-shadow: 10px 25px 50px -6px rgba(0, 0, 0, 0.25); 
}
...
.ease-out-enter {
  animation: easeOutEnter 800ms ease-out;
}
